...
//!
//! ```rust,no_run
//! use pgsafe::migrate::{MigrationSource, MigrationsConfig, run, status};
//! use std::path::Path;
//!
...
/// behavior). Re-exported from `pgsafe_core` so consumers of the runtime
/// crate don't need to depend on `pgsafe_core` directly.
pub use pgsafe_core::config::{ConfigError, MigrationsConfig};
