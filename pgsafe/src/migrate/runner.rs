...
/// #     source: &pgsafe::migrate::MigrationSource,
/// #     config: &pgsafe::migrate::MigrationsConfig) -> Result<(), pgsafe::Error> {
/// let statuses = pgsafe::migrate::status(client, source, config).await?;
/// for s in &statuses {
///     if !s.applied {
...
pub async fn run(
    ...
    match (&result, release) {
        ...
        (Err(_), Err(rel_err)) => {
            eprintln!("pgsafe: failed to release advisory lock: {rel_err}");
            result
        }
        (Err(_), Ok(_)) => result,
    }
                    ...
                    if config.fail_on_drift {
                        return Err(crate::Error::Migration(format!(
                            "migration '{}' has been modified since it was applied; \
                             set [package.metadata.pgsafe.migrations] fail_on_drift = false \
...
///
/// ```rust,no_run
/// use pgsafe::migrate::{MigrationSource, MigrationsConfig, status};
/// use std::path::Path;
///
/// # async fn example(client: &tokio_postgres::Client) -> Result<(), pgsafe::Error> {
/// let source = MigrationSource::from_dir(Path::new("./migrations"))?;
...
///
/// ```rust,no_run
/// use pgsafe::migrate::{MigrationSource, MigrationsConfig, revert};
/// use std::path::Path;
///
/// # async fn example(client: &mut tokio_postgres::Client) -> Result<(), pgsafe::Error> {
/// let source = MigrationSource::from_dir(Path::new("./migrations"))?;
