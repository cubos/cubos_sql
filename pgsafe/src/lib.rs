...
//!
//! `pgsafe` checks your SQL queries against a real PostgreSQL schema at compile time,
...
//! under `[package.metadata.pgsafe.databases.<name>]` (same shape: `database`,
...
//! use pgsafe::sql;
//!
//! # async fn example(pool: &deadpool_postgres::Pool) -> Result<(), pgsafe::Error> {
...
//! # Multiple databases
//!
//! Declare a named database under `[package.metadata.pgsafe.databases.<name>]`
...
//!
//! ```rust,ignore
//! // Default database (uses the top-level [pgsafe.database] config)
//! let users = sql!(app_pool, "SELECT id, name FROM users")
//!     .fetch_all().await?;
//!
//! // Named database (uses [pgsafe.databases.analytics.*])
//! let metrics = sql!(db = analytics, warehouse_pool,
//!     "SELECT kind, total FROM daily_metrics")
//!     .fetch_all().await?;
//! ```
//!
//! `pgsafe` does not multiplex pools — the executor (`app_pool` /
...
//! use std::path::Path;
//!
//! # async fn example() -> Result<(), pgsafe::Error> {
...
//! # Pool setup
//!
//! `pgsafe` does **not** create or manage connection pools — that is the
...
//!
//! ```toml
//! pgsafe = { version = "0.1", default-features = false, features = ["bb8"] }
//! bb8-postgres = "0.9"
//! ```
//!
//! # The `Executor` trait
...
//! `[package.metadata.pgsafe.{domains, enums, types}]` sections shown above.
//! See `pgsafe_macros::pg_type_map` for the full set of recognized built-ins.
//!
...
//! pre-existing struct via `[package.metadata.pgsafe.types]` makes the
...
/// Build-script helpers — re-exported from `pgsafe_core`.
///
/// Call [`build::track_migrations`] from your crate's `build.rs` so the `sql!`
/// macro is re-checked whenever a migration file changes.
pub use pgsafe_core::build;

pub use pgsafe_macros::FromRow;
/// Re-export the `embed_migrations!` macro from `pgsafe_macros`.
///
/// See the [`macro@embed_migrations`] documentation for usage details.
pub use pgsafe_macros::embed_migrations;
/// Re-export the `sql!` macro from `pgsafe_macros`.
///
/// See the [`macro@sql`] documentation for full syntax, examples, and
/// configuration details.
pub use pgsafe_macros::sql;
