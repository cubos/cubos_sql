//! Configuration types for `pgsafe`.
//!
//! The configuration is read from the `[package.metadata.pgsafe]` section
...
//! `Vec<RustType>`.
//!
//! All fields have sensible defaults. The `[package.metadata.pgsafe]` section
...
/// Top-level configuration for `pgsafe`, read from `[package.metadata.pgsafe]` in `Cargo.toml`.
///
...
///
/// Configured under `[package.metadata.pgsafe.databases.<name>]` in `Cargo.toml`.
...
/// Wrapper to extract `[package.metadata.pgsafe]` from a full Cargo.toml.
...
/// Errors that can occur when loading or parsing the `pgsafe` configuration.
///
...
pub enum ConfigError {
    ...
    #[error("unknown database '{0}' — not found in [package.metadata.pgsafe.databases]")]
...
impl std::str::FromStr for Config {
    ...
    fn from_str(content: &str) -> Result<Self, Self::Err> {
        ...
        let config = cargo.package.metadata.pgsafe;
...
[package.metadata.pgsafe.databases.analytics]
[package.metadata.pgsafe.databases.analytics.database]
migrations = "./migrations/analytics"

[package.metadata.pgsafe.databases.analytics.migrations]
