...
//!
//! A proc macro that reads files (like `pgsafe`'s `sql!`) cannot, on stable
...
//! ```
//!
//! And declare `pgsafe` as a build dependency:
//!
//! ```toml
//! # Cargo.toml
//! [build-dependencies]
...
/// `[package.metadata.pgsafe]` from the crate's `Cargo.toml`, then, for
...
/// `[package.metadata.pgsafe]` section is *not* an error — defaults are used
...
pub fn track_migrations() {
    ...
    let config = Config::from_cargo_toml(&cargo_toml).unwrap_or_else(|e| {
        panic!(
            "pgsafe::build::track_migrations: failed to load {}: {e}",
            cargo_toml.display(),
        )
    });
...
mod tests {
    ...
    fn fingerprint_handles_missing_dir() {
        let dirs = vec![PathBuf::from("/nonexistent/pgsafe/migrations")];
