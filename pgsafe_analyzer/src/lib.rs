//! Static SQL type and nullability analyzer for `pgsafe`.
//!
...
/// Re-exports of types defined in `pgsafe_core` but used pervasively by
/// the analyzer. Kept here so downstream crates (and tests) can depend only
/// on `pgsafe_analyzer`.
pub(crate) mod qualified_name {
    pub use pgsafe_core::QualifiedName;
}
...
pub use pgsafe_core::{ParseQualifiedNameError, QualifiedName};
pub use resolve::{
