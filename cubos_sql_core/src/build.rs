...
//!
//! A proc macro that reads files (like `cubos_sql`'s `sql!`) cannot, on stable
...
//! ```
//!
//! And declare `cubos_sql` as a build dependency:
//!
//! ```toml
//! # Cargo.toml
//! [build-dependencies]
...
const FINGERPRINT_ENV: &str = "CUBOS_SQL_MIGRATIONS_FINGERPRINT";

/// Wire migration files into Cargo's change-detection so `sql!` is re-checked
/// whenever they change.
///
/// Call this from your crate's `build.rs`. It reads
/// `[package.metadata.cubos_sql]` from the crate's `Cargo.toml`, then, for
...
/// `[package.metadata.cubos_sql]` section is *not* an error — defaults are used
...
pub fn track_migrations() {
    ...
    let config = Config::from_cargo_toml(&cargo_toml).unwrap_or_else(|e| {
        panic!(
            "cubos_sql::build::track_migrations: failed to load {}: {e}",
            cargo_toml.display(),
        )
    });
...
mod tests {
    ...
    fn fingerprint_handles_missing_dir() {
        let dirs = vec![PathBuf::from("/nonexistent/cubos_sql/migrations")];
