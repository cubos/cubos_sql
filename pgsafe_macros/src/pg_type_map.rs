...
//!
//! The analyzer produces a [`pgsafe_analyzer::Type`] that describes a
...
//! `[package.metadata.pgsafe.{domains, enums, types}]` take precedence
//! over these tables.

/// Built-in PostgreSQL types, keyed by `(schema, pg_name)`.
///
/// Schema is always `pg_catalog` for true built-ins. Arrays are NOT listed
/// here — they are resolved generically via [`pgsafe_analyzer::Type::Array`]
...
/// analyzer's [`pgsafe_analyzer::Type::Basic`] (and friends) `extension`
