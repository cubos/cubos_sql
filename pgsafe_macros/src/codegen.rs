...
//!
//! Receives an [`AnalyzedQuery`] from `pgsafe_analyzer` and produces a
...
use pgsafe_core::config::ResolvedConfig;
...
/// Wrap the SQL in a `SELECT * FROM (<sql>) AS __pgsafe_limit LIMIT 2`
...
fn wrap_with_limit(sql: &str, can_run_as_subquery: bool) -> Option<String> {
    can_run_as_subquery.then(|| format!("SELECT * FROM ({sql}) AS __pgsafe_limit LIMIT 2"))
}
            ...
            Err(syn::Error::new(
                ...
                format!(
                    "no Rust mapping for PostgreSQL type {qn} — add it to \
                     [package.metadata.pgsafe.types] in your Cargo.toml"
                ),
            ))
        }
        Type::Range { subtype, .. } => {
...
fn emit_one_record(
    ...
    let pg = quote! { ::pgsafe::__private::tokio_postgres::types };
    let box_err = quote! {
    ...
    Ok(quote! {
        ...
        impl<'__pgsafe_a> #pg::FromSql<'__pgsafe_a> for #ident {
            fn from_sql(
                _ty: &#pg::Type,
                __raw: &'__pgsafe_a [u8],
            ) -> ::std::result::Result<Self, #box_err> {
                let mut __reader = ::pgsafe::__private::RecordReader::new(__raw)?;
                #from_sql_reads
            ...
            fn accepts(__ty: &#pg::Type) -> bool {
                ::pgsafe::__private::record_accepts(__ty)
            }
        }
    })
}
    ...
    let ts = quote! {
        {
            ...
            impl<__E: pgsafe::Executor> __pgsafe_query<__E> {
                ...
                async fn fetch_all(self) -> ::std::result::Result<::std::vec::Vec<__sql_output>, pgsafe::Error> {
                    let __rows = pgsafe::Executor::query(
                        &self.__executor,
                        #sql_str,
                        &[#params_slice],
                    ).await?;
                ...
                async fn fetch_one(self) -> ::std::result::Result<__sql_output, pgsafe::Error> {
                    let __rows = pgsafe::Executor::query(
                        &self.__executor,
                        #sql_limited,
                        &[#params_slice],
                    ...
                    let __row = __iter.next()
                        .ok_or_else(|| pgsafe::Error::NoRows)?;
                    if __iter.next().is_some() {
                        return ::std::result::Result::Err(pgsafe::Error::TooManyRows);
                    }
                    ::std::result::Result::Ok(__sql_output {
                        #row_mapping
                    })
                ...
                async fn fetch_optional(self) -> ::std::result::Result<::std::option::Option<__sql_output>, pgsafe::Error> {
                    let __rows = pgsafe::Executor::query(
                        &self.__executor,
                        #sql_limited,
                        &[#params_slice],
                ...
                async fn fetch_all_as<__T: pgsafe::FromRow>(self) -> ::std::result::Result<::std::vec::Vec<__T>, pgsafe::Error> {
                    let __rows = pgsafe::Executor::query(
                        &self.__executor,
                        #sql_str,
                        &[#params_slice],
                ...
                async fn fetch_one_as<__T: pgsafe::FromRow>(self) -> ::std::result::Result<__T, pgsafe::Error> {
                    let __rows = pgsafe::Executor::query(
                        &self.__executor,
                        #sql_limited,
                        &[#params_slice],
                ...
                async fn fetch_optional_as<__T: pgsafe::FromRow>(self) -> ::std::result::Result<::std::option::Option<__T>, pgsafe::Error> {
                    let __rows = pgsafe::Executor::query(
                        &self.__executor,
                        #sql_limited,
                        &[#params_slice],
...
fn generate_spread(
    ...
    let query_preamble = quote! {
        ...
        let mut __params: Vec<Box<dyn ::pgsafe::__private::tokio_postgres::types::ToSql + Sync>>
            = Vec::with_capacity(#capacity_expr);
        #regular_param_pushes
        #spread_param_pushes
        let __params_ref: Vec<&(dyn ::pgsafe::__private::tokio_postgres::types::ToSql + Sync)>
    ...
    let ts = quote! {
        {
            ...
            #[allow(non_camel_case_types)]
            struct __pgsafe_query<'__s, __E: pgsafe::Executor, #(#spread_generic_types,)*> {
                __executor: __E,
            ...
            impl<'__s, __E: pgsafe::Executor, #(#spread_generic_types,)*>
                __pgsafe_query<'__s, __E, #(#spread_generic_types,)*>
            {
                async fn fetch_all(self) -> ::std::result::Result<::std::vec::Vec<__sql_output>, pgsafe::Error> {
                    #query_preamble
                    if __any_empty {
                    ...
                    let __rows = pgsafe::Executor::query(&self.__executor, &__sql, &__params_ref).await?;
                    __rows.into_iter().map(|__row| {
                ...
                async fn fetch_one(self) -> ::std::result::Result<__sql_output, pgsafe::Error> {
                    #query_preamble
                    if __any_empty {
                        return ::std::result::Result::Err(pgsafe::Error::NoRows);
                    }
                    let __rows = pgsafe::Executor::query(&self.__executor, &__sql, &__params_ref).await?;
                    let mut __iter = __rows.into_iter();
                    let __row = __iter.next().ok_or_else(|| pgsafe::Error::NoRows)?;
                    if __iter.next().is_some() {
                        return ::std::result::Result::Err(pgsafe::Error::TooManyRows);
                    }
                    ::std::result::Result::Ok(__sql_output { #row_mapping })
                ...
                async fn execute(self) -> ::std::result::Result<u64, pgsafe::Error> {
                    ...
                    pgsafe::Executor::execute(&self.__executor, &__sql, &__params_ref).await
                }
...
fn build_fetch_value_method(
    ...
    Ok(quote! {
        async fn fetch_value(self) -> ::std::result::Result<#return_type, pgsafe::Error> {
            let __v = self.fetch_one().await?;
            ::std::result::Result::Ok(__v.#field_name)
        }

        async fn fetch_value_optional(self) -> ::std::result::Result<#optional_return_type, pgsafe::Error> {
            #optional_body
        }
    })
}
...
fn build_field_type_and_value<P: TypedParam>(
    ...
    let value_expr = match (accepts_into_string, mapping.accepts_iter, is_nullable) {
        (true, _, true) => {
            quote! {
                ::pgsafe::__private::IntoOptionString::into_option_string(#value_expr)
            }
        }
        (true, _, false) => quote! { Into::<String>::into(#value_expr) },
        (_, true, false) => {
            // Vec<T> with a plain element — accept any IntoIterator<Item: Into<T>>.
            quote! { ::pgsafe::__private::into_flex_vec(#value_expr) }
        }
        (false, _, true) => {
...
fn push_param<P: TypedParam>(
    ...
    let is_nullable = param.nullable();
    let to_sql_ty = quote! {
        Box<dyn ::pgsafe::__private::tokio_postgres::types::ToSql + Sync>
    };
                ...
                quote! {
                    __params.push(Box::new(match &#accessor {
                        Some(__v) => Some(::serde_json::to_value(__v)
                            .map_err(|e| pgsafe::Error::Serialize(
...
fn build_params_slice(
    ...
    let to_sql = quote! {
        &(dyn ::pgsafe::__private::tokio_postgres::types::ToSql + Sync)
    };
                ...
                if nullable {
                    quote! {
                        &self.#field_name.as_ref().map(|__v|
                            ::pgsafe::__private::EnumString(__v.to_string()))
                            as #to_sql,
                    }
                } else {
                    quote! {
                        &::pgsafe::__private::EnumString(self.#field_name.to_string())
                            as #to_sql,
                    }
                }
            }
...
#[derive(Clone, Copy)]
enum DecodeCtx {
    /// Inside the `sql!` row-mapping closure — errors are [`pgsafe::Error`].
    Column,
...
impl DecodeCtx {
    ...
    fn wrap_err(self, msg: TokenStream) -> TokenStream {
        match self {
            DecodeCtx::Column => quote! { pgsafe::Error::Deserialize(#msg) },
...
fn decode_value(
    ...
    let json_hint = || {
        ...
        quote! { &::pgsafe::__private::tokio_postgres::types::Type::#variant }
    };
    match &mapping.strategy {
        ...
        DeserStrategy::EnumAsString { target } => {
            ...
            if nullable {
                let rd = read(
                    quote! { ::std::option::Option<::pgsafe::__private::EnumString> },
                    None,
                );
                Ok(quote! {
                    match #rd {
                ...
                let rd = read(quote! { ::pgsafe::__private::EnumString }, None);
                Ok(quote! {
        ...
        DeserStrategy::VecOfJsonbDomain { inner } => {
            ...
            let hint = Some(quote! {
                &::pgsafe::__private::tokio_postgres::types::Type::JSONB_ARRAY
            });
            let map = quote! {
                __vs.into_iter()
        ...
        DeserStrategy::VecOfEnumAsString { inner } => {
            ...
            if nullable {
                let rd = read(
                    quote! { ::std::option::Option<::std::vec::Vec<::pgsafe::__private::EnumString>> },
                    None,
                );
                Ok(quote! {
                    match #rd {
                ...
                let rd = read(
                    quote! { ::std::vec::Vec<::pgsafe::__private::EnumString> },
                    None,
                );
                Ok(quote! { { let __vs = #rd; #map } })
            }
