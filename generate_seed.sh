#!/usr/bin/env bash
#
# Generates typedpg_analyzer/src/seed.json from a clean PostgreSQL instance.
#
# This script:
...
#
# The seed.json file is written to typedpg_analyzer/src/seed.json
# and should be committed to the repository.

set -euo pipefail

PG_IMAGE="${1:-postgres:18}"
CONTAINER_NAME="typedpg_seed_$$"
...
cargo run -p typedpg_seed -- "$CONN"

echo "==> Done! seed.json has been updated."
echo "    Remember to commit typedpg_analyzer/src/seed.json"
