...
fn assert_param_types(db: &Database, sql: &str, expected: &[&str]) {
    let info = db.analyze(sql, &default_config()).unwrap();
    assert_eq!(
        info.params.len(),
        expected.len(),
...
fn goal_limit_and_offset_params() {
    ...
    assert_param_types(&db, sql, &["i64", "i64"]);
}
...
fn goal_limit_function_of_param() {
    ...
    assert_param_types(&db, sql, &["String"]);
}
...
fn goal_where_bool_param() {
    ...
    assert_param_types(&db, sql, &["bool"]);
}
...
fn goal_where_comparison_infers_column_type() {
    ...
    assert_param_types(&db, sql, &["i64", "String", "i32"]);
}
...
fn goal_where_and_or_bool_propagation() {
    ...
    assert_param_types(&db, sql, &["bool", "bool", "bool"]);
}
...
fn goal_insert_values_params() {
    ...
    assert_param_types(&db, sql, &["String", "String", "i32"]);
}
...
fn goal_update_set_params() {
    ...
    assert_param_types(&db, sql, &["String", "i32", "i64"]);
}
...
fn goal_cast_types_param() {
    ...
    assert_param_types(&db, sql, &["String", "i32"]);
}
...
fn goal_coalesce_all_params() {
    ...
    assert_param_types(&db, sql, &["String", "String"]);
}
...
fn goal_param_insert_with_returning() {
    ...
    assert_param_types(&db, sql, &["i64", "String", "String"]);
}
...
fn goal_operator_param_both_sides() {
    ...
    let db = setup();
    assert_param_types(&db, "SELECT $p1 > $p2", &["String", "String"]);
}
...
fn goal_insert_multiple_value_rows() {
    ...
    assert_param_types(&db, sql, &["String", "String", "String", "String"]);
}
...
fn goal_numeric_promotion_int4_with_int8() {
    ...
    assert_param_types(&db, sql, &["i64", "i32"]);
}
...
// on `pg_type_oid` to distinguish types that share the same Rust mapping.

/// Assert the single param of `sql` resolves to the expected PG type OID.
fn assert_single_param_oid(db: &Database, sql: &str, expected_oid: u32) {
    let info = db.analyze(sql, &default_config()).unwrap();
    assert_eq!(info.params.len(), 1, "expected exactly one param in: {sql}");
    assert_eq!(
        info.params[0].pg_type_oid, expected_oid,
        "param PG OID mismatch for: {sql}"
    );
}

/// pg_catalog OIDs used by the tests below.
...
fn preferred_type_text_wins_over_bytea_in_reverse_registration_order() {
    ...
    .unwrap();
    assert_single_param_oid(&db, "SELECT pick($p)", OID_TEXT);
}
...
fn preferred_type_ignores_non_string_category_when_only_non_preferred_in_string() {
    ...
    // would return bytea (OID 17). The filter keeps only bpchar (OID 1042).
    let _ = OID_BYTEA;
    let _ = OID_VARCHAR;
    assert_single_param_oid(&db, "SELECT pick($p)", OID_BPCHAR);
}
